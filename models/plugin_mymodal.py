modal_template = """
<div id="%(id)s" style="position:absolute;display:none;">
<div
...
<div style="position:absolute;
            ...
            z-index:1002;
            overflow:auto;">
<span style="font-weight:bold">%(title)s</span>
<span style="float:right">
  [<a href="#" onclick="jQuery(\'#%(id)s\').hide();return false;">%(close)s</a>]
</span>
<hr/>
<div id="%(id)s_content" _style="background-color:white;-moz-opacity:1.0;opacity:1.0;z-index:1003;">%(content)s</div>
</div>
</div>
"""
...
class PluginMModal:    
    ...
    def __init__(self, title, content,
        ...
        self.height=height
    def xml(self):
        return modal_template % dict(title=self.title,content=self.content,close=self.close,id=self.id,left=(100-self.width)/2,top=(100-self.height)/2,width=self.width,height=self.height)
    def link(self,message):
        script=''
        if self.callback:
            script="ajax('%s',[],'%s_content');" % (self.callback,self.id)
