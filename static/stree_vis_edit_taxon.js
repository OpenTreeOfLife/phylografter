...
function stree_vis_edit_taxonInit( p ) {
    ...
    r.path( pathString ).attr( { stroke: 'gray', "stroke-width": 2 } );
...
function buildTreePathString( p ) {
    ...
    if( p.node.label ) { labels.push( { x: p.node.coords.unscaled.x, y: p.node.coords.y, text: p.node.label, nodeId: p.node.id, isleaf: p.node.children.length==0 } ); }
...
function asyncRenderLabel( p ) {
    ...
    attr = { stroke: 'black',
	     ...
	     "text-anchor": anchor }
    if ( label.isleaf ) {
	    attr["text-anchor"] = "start"; x = label.x-pad;
    }
    var raphaelLabel = p.canvas.text( x, label.y, label.text )
                               .attr( attr );
