...
# the badgerfish (http://badgerfish.ning.com - original site?) rules
# for mapping xml to json
...
# local naming convention: get_XXX are DAL queries, generally returning an id
# or list of ids', everything else is generating dicts or lists that 
# correspond to BadgerFish  mappings of Elements

# Output has been tested using the translator on 
# http://dropbox.ashlock.us/open311/json-xml/ 
# and validating the resulting xml with the validator at nexml.org
...
def xmlNameSpace():
    """
    namespace definitions for nexml; will be value of xmlns attribute in header 
    (per badgerfish treatment of namespaces)
    """
    return {
...
def doi_meta_for_study(study_row):
    """
    generates returns an ot:studyPublication metadata element if a 
    (possibly incomplete) doi or a suitable URI is available for 
    the study, else nothing
    """
    doi = study_row.doi
    if doi:
        if (doi.startswith(DOI_PREFIX)):
            pass  # fine, leave as is
        elif (doi.startswith('http://www.')):
            # not a doi, but an identifier of some sort
            pass
        elif (doi.startswith('doi:')):  
            # splice the http prefix on
           doi = DOI_PREFIX + doi[4:]
        elif not(doi.startswith('10.')):  
            # not a doi, or corrupted, treat as blank
            return
        else:
           doi = DOI_PREFIX + doi
...
def get_otu_rows_for_study(study_row,db):
    ...
    '''
    return db.executesql('SELECT otu.id, otu.label, otu.ott_node, ott_node.id, ott_node.name, otu.tb_nexml_id, otu.other_metadata FROM otu LEFT JOIN ott_node ON (otu.ott_node = ott_node.id) WHERE (otu.study = %d);' % study_row.id)
...
def otu_elt(otuRec):
    ...
    otu_id,label,ott_node,accepted_uid,name,tb_name,other_meta = otuRec
