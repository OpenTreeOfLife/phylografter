...
write_metadata(metadata_dir, "invocation", "%s\n" % ' '.join([shell_escape_arg(i) for i in invocation]))
try:
    write_metadata(metadata_dir, "launcher_pid", "%s\n" % str(os.getpid()))
    write_metadata(metadata_dir, "env", "%s\n" % repr(os.environ))
    write_metadata(metadata_dir, "stdioe", "%s\n%s\n%s\n" % (stdinpath, stdoutpath, stderrpath))
except:
    pass
...
write_metadata(metadata_dir, "pid", "%s\n" % str(proc.pid))
if in_obj:
    proc.stdin.write(in_obj.read())
proc.stdin.close()
proc.wait()
rc = proc.returncode
write_metadata(metadata_dir, "returncode", "%s\n" % str(rc))
if rc == 0:
